C--SEAWAT: REMOVED SUBROUTINES UCOLNO AND URWORD.  CONTAINED IN MODFLOW-2000 SOURCE
C--SEAWAT: MODIFED RARRAY TO READ SINGLE PRECISION BINARY ARRAYS
C--SEAWAT: CHANGED FILESPEC.INC TO filespec.inc
C--SEAWAT: MODIFIED RARRAY TO READ (FREE) FORMAT
C--SEAWAT: MODIFIED IARRAY TO READ (FREE) FORMAT
C
C
...
C--SEAWAT: DECLARING VARIABLES FOR (FREE) READING
      CHARACTER*20 CNTRL
      INTEGER ICOL,ISTART,ISTOP
      REAL R
C--SEAWAT: END DECLARING
...
C--SEAWAT: MODIFIED IARRAY TO READ (FREE) FORMAT
       ICOL=1
       CALL URWORD(FMTIN,ICOL,ISTART,ISTOP,1,N,R,IOUT,IN)
       CNTRL=FMTIN(ISTART:ISTOP)
        DO 270 I=1,II
          IF(CNTRL.EQ.'(FREE)') THEN
            READ(IREAD,*) (IA(J,I),J=1,JJ)
          ELSE
...
C--SEAWAT: DECLARING VARIABLES FOR (FREE) READING
      ...
      INTEGER ICOL,ISTART,ISTOP
      REAL R
C--SEAWAT: NEED REAL*4 VARIABLE TO READ SINGLE PRECISION FROM BINARY FILE
      REAL*4 T
C
C--READ ARRAY CONTROL RECORD
C  =========================
...
C
C--READ AN UNFORMATTED DUMMY RECORD FIRST.
C--SEAWAT: MODIFIED THIS SECTION TO READ SINGLE PRECISION BINARY ARRAY
C--SEAWAT:        READ(-IREAD) NTRANS,KSTP,KPER,TOTIM,TEXT,NC,NR,ILAY
C--SEAWAT:        READ(-IREAD) A
C--SEAWAT: THIS SECTION MAY NEED TO BE CHANGED IF FORM='UNFORMATTED'
        READ(-IREAD) NTRANS,KSTP,KPER,T,TEXT,NC,NR,ILAY
        DO I=1,NR
        DO J=1,NC
          READ(-IREAD) T
        A(J,I)=T
