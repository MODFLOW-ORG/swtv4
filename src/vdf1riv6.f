...
C
C
      SUBROUTINE VDF1RIV6SSMDENSE(IOUT,NRIVER,MXRIVR,RIVR,NRIVVL,
     1                          MXSS,NSS,SS,NCOMP,SSMC,NSSVL)
C-----VERSION 09SEPTEMBER2009 IMT1RIV6SSMDENSE
C     ******************************************************************
C     USE AUXILIARY SPACE TO STORE DENSITY
C     ******************************************************************
C
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C
      IQ=4
C
C-----RETURN FROM THIS ROUTINE IF DENSE NOT STORED IN RIVR
      ILOC=0
      DO I=1,5
        IF(RIVAUX(I).EQ.'RIVSSMDENSE') ILOC=I+6
      ENDDO   
      IF(ILOC.EQ.0) RETURN
      WRITE(IOUT,100)
C
C-----RETURN IF THERE ARE NO RIVERS FOR THIS STRESS PERIOD
      IF(NRIVER.LE.0) RETURN
C
C
C-----LOOP THROUGH EACH RIVER AND STORE DENSITY IN RIVR ARRAY
      DO L=1,NRIVER
        IL=RIVR(1,L)
        IR=RIVR(2,L)
        IC=RIVR(3,L)
        RIVR(ILOC,L)=SSMDENSE(IC,IR,IL,IQ,MXSS,NSS,SS,NCOMP,SSMC)
      ENDDO
C
  100 FORMAT(1X,'THE RIVER AUXILIARY VARIABLE RIVSSMDENSE IS BEING ',
