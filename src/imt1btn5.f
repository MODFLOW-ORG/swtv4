C--SEAWAT: CHANGED FILESPEC.INC TO filespec.inc
C
      SUBROUTINE IMT1BTN5OPEN(INUNIT,IOUT,INBTN,INADV,INDSP,INSSM,INRCT,
...
C
     ...
     &              INTOB,INFTL,ILIST,IBTN,IFTL,IFLEN,IFTLFMT,
     &              MXTRNOP,iUnitTRNOP
      LOGICAL       LOP
...
C
C
      SUBROUTINE IMT1BTN5DF(INBTN,IOUT,ISUM,ISUM2,NCOL,NROW,NLAY,NPER,
...
C
C
      SUBROUTINE IMT1BTN5AL(INBTN,IOUT,ISUM,ISUM2,NCOL,NROW,NLAY,NCOMP,
...
C
C
      SUBROUTINE IMT1BTN5RP(IN,IOUT,IUCN,IUCN2,IOBS,IMAS,ICNF,ICBM,NCOL,
...
C
C
      SUBROUTINE IMT1BTN5ST(IN,IOUT,NSTP,MXSTP,TSLNGH,DT0,MXSTRN,
...
C
C
C--SEAWAT: ADDED FIRSTDT TO ARGUMENT LIST
      SUBROUTINE IMT1BTN5AD(NTRANS,MXTRNOP,iUnitTRNOP,TIME1,TIME2,HT2,
     & DELT,KSTP,NSTP,MXPRS,TIMPRS,DT0,MXSTRN,MIXELM,DTRACK,DTRACK2,
     ...
     & PRSITY2,RETA2,ISOTHM,TMASIO,RMASIO,TMASS,FIRSTDT)
C **********************************************************************
...
C--SEAWAT: DECLARED FIRSTDT AS REAL
      REAL      FIRSTDT     
...
C
C--SEAWAT: IF DTRANS IS ZERO, SET TO FIRSTDT
      IF(DTRANS.EQ.0.) DTRANS=FIRSTDT
...
C
      UPDLHS=.TRUE.
C--SEAWAT: FOR NOW, COMMENT FOLLOWING LINES SO RHS IS ALWAYS UPDATED
C--SEAWAT:      IF(ABS(DTRANS-DTOLD).LT.ABS(DTRANS+DTOLD)*EPSILON
C--SEAWAT:     & .AND. NTRANS.GT.1) UPDLHS=.FALSE.
C
...
C
C
      SUBROUTINE IMT1BTN5SV(NCOL,NROW,NLAY,NCOMP,ICOMP,ICBUND,CNEW,
     & CWGT,CINACT,RMASIO)
C **************************************************************
...
C
C
      SUBROUTINE IMT1BTN5BD(KPER,KSTP,NTRANS,NCOL,NROW,NLAY,NCOMP,ICOMP,
...
C
C
      SUBROUTINE IMT1BTN5OT(NCOL,NROW,NLAY,KPER,KSTP,NTRANS,NCOMP,ICOMP,
...
C--DISSOLVED PHASE CONCENTRATION TO FILE [MT3Dnnn.UCN]
        DO K=1,NLAY
C--SEAWAT CHANGED FOLLOWING TWO STATEMENTS TO WRITE SINGLE PRECISION
          WRITE(IUCN+ICOMP) NTRANS,KSTP,KPER,REAL(TIME2,4),TEXT,NCOL,
     &                      NROW,K
          WRITE(IUCN+ICOMP) ((REAL(CNEW(J,I,K,ICOMP),4),J=1,NCOL),
     &                      I=1,NROW)
        ENDDO
C
C--SORBED/IMMOBILE PHASE CONCENTRATIONS TO FILE [MT3DnnnS.UCN]
        IF(ISOTHM.GT.0) THEN
          DO K=1,NLAY
C--SEAWAT CHANGED FOLLOWING TWO STATEMENTS TO WRITE SINGLE PRECISION
            WRITE(IUCN2+ICOMP) NTRANS,KSTP,KPER,REAL(TIME2,4),TEXT,NCOL,
     &                         NROW,K
            WRITE(IUCN2+ICOMP) ((REAL(SRCONC(J,I,K,ICOMP),4),J=1,NCOL),
     &                         I=1,NROW)
          ENDDO
        ENDIF
      ENDIF
...
C
C
      SUBROUTINE IMT1BTN5FM(NCOL,NROW,NLAY,NCOMP,ICOMP,ICBUND,CADV,COLD,
