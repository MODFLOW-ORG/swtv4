C--SEAWAT: THIS FILE HAS BEEN MODIFIED TO WORK WITH THE SEAWAT PROGRAM.
C--SEAWAT: BECAUSE ALL VARIABLES IN SEAWAT ARE DOUBLE PRECISION, ALL
C--SEAWAT: LMT SUBROUTINES WERE MODIFIED TO WRITE SINGLE PRECISION, IF
C--SEAWAT: THE OUTPUT FILE IS FOR A SUBSEQUENT MT3DMS SIMULATION, OR 
C--SEAWAT: DOUBLE PRECISION IF THE OUTPUT FILE IS THE TEMPORARY FILE 
C--SEAWAT: USED BY SEAWAT TO PASS FLOW INFORMATION TO MT3DMS SUBROUTINES.
C--SEAWAT: LANGEVIN UPDATED WITH MODFLOW VERSION 1.17.00 (09/14/2006)
C
...
C
     ...
     & SC1,SC2,HOLD,BOTM,NBOTM,NCOL,NROW,NLAY,KSTP,KPER,BUFF,IOUT,INUHF,
     & DH,QX,QY,QZ,QSTO,MXSS,NTSS,NSS,SS,ICBUND)
C *********************************************************************
...
C
     ...
     &  NCOL,NROW,NLAY,KSTP,KPER,IOUT,INUHF,MXSS,NTSS,NSS,SS,
     &  ICBUND)
C *********************************************************************
...
C
     ...
     & KPER,IFHBD4,NFLWDIM,IOUT,INUHF,MXSS,NTSS,NSS,SS,ICBUND)
C **********************************************************************
...
C--WRITE AN IDENTIFYING HEADER
      IQ=23
      IF(ILMTFMT.EQ.0) THEN
        IF(IOUT.NE.INUHF) WRITE(IOUT) KPER,KSTP,NCOL,NROW,NLAY,TEXT,NFLW
      ELSEIF(ILMTFMT.EQ.1) THEN
        WRITE(IOUT,*) KPER,KSTP,NCOL,NROW,NLAY
...
C
     ...
     &  BUFF,KSTP,KPER,NRES,NRESOP,NCOL,NROW,NLAY,IOUT,INUHF,MXSS,NTSS,
     &  NSS,SS,ICBUND)
C **********************************************************************
...
C
     ...
     &  NLAY,KSTP,KPER,IOUT,ILMTHEAD,INUHF,MXSS,NTSS,NSS,SS,ICBUND)
C **********************************************************************
...
C--PROCESS WELL LIST
      DO m = 1,nwell2
        ...
        ic = mod((n-1),ncol) + 1
        IDwell = ifrl(well2(18,m))  !!IDwell contained in well2(18,m); cdl 4/19/05
        Q = well2(17,m)
C
C--IF CELL IS EXTERNAL Q=0
...
C--SAVE TO OUTPUT FILE
        IF(ILMTFMT.EQ.0) THEN
C--SEAWAT: PASS THROUGH MEMORY OR WRITE IN SINGLE PRECISION
          IF(IOUT.EQ.INUHF) THEN
            IQ=27
            CALL PASSGS(IL,IR,IC,Q,IDWELL,QSW,IQ,NCOL,NROW,NLAY,
     +                  MXSS,NTSS,NSS,SS,ICBUND)
          ELSE
            WRITE(IOUT) IL,IR,IC,REAL(Q,4),IDwell,REAL(QSW,4)
          ENDIF
        ELSEIF(ILMTFMT.EQ.1) THEN
...
C
     ...
     & NCOL,NROW,NLAY,KSTP,KPER,BUFF,IOUT,NETSEG,PXDP,PETM,NSEGAR,INUHF,
     & IEVTMT3D,EVTRMT3D)
C ********************************************************************
...
C        IF (HHNEW.GT.EEL) THEN
          CC=C
          CEL=C*EL
          QQ=0.
          IF (HHNEW.GT.EEL) QQ=CEL - CC*HHNEW
          Q=QQ
          ILR=0
          IF (IDRTFL.GT.0) THEN
            ILR = DRTF(6,L)
