C--SEAWAT: CHANGED COMMON /FC/ TO COMMON /FCMT3D/ TO AVOID CONFLICT
C--SEAWAT: ADDED IMT1 PREFIX TO PRIMARY SUBROUTINES
C--SEAWAT: ADDED ERROR AND STOP IF SSM IS OUTSIDE OF GRID BOUNDARIES
C
...
C
        ...
        ELSEIF(IQ.LT.1.OR.IQ.GT.100) THEN
          ...
          CALL USTOP(' ')          
        ENDIF
C--SEAWAT: ADDING ERROR MESSAGE AND STOP FOR SSM OUTSIDE OF BOUNDS
      IF(KK.LT.1.OR.KK.GT.NLAY
     +  .OR.II.LT.1.OR.II.GT.NROW
     +  .OR.JJ.LT.1.OR.JJ.GT.NCOL) THEN
        WRITE(IOUT,81) NUM,KK,II,JJ
        CALL USTOP(' ')
   81 FORMAT(/1X,'ERROR: SSM NUMBER ',I10,' IS OUTSIDE OF GRID BOUNDS',
     &       /1X,'       AT K I J ',3I10)
      ENDIF
C--SEAWAT: END ERROR MESSAGE
        SS(1,NUM)=KK
        SS(2,NUM)=II
        SS(3,NUM)=JJ
        SS(4,NUM)=CSS
...
C
     ...
     &          CTMP,QSS,QCTMP,DELR,DELC,DH,QSTO,A,RHS,COLD
     ...
     &          A(NODES),RHS(NODES),COLD(NCOL,NROW,NLAY,NCOMP)
      COMMON /FCMT3D/FWEL,FDRN,FRCH,FEVT,FRIV,FGHB,
     &           FSTR,FRES,FFHB,FIBS,FTLK,FLAK,FMNW,FDRT,FETS,FUSR
C
...
C--SEAWAT: mt3d bug      A(N)=A(N)+QSTO(J,I,K)*DELR(J)*DELC(I)*DH(J,I,K)
...
C
     ...
     & DH,IRCH,RECH,CRCH,IEVT,EVTR,CEVT,MXSS,NTSS,SS,SSMC,SSG,
     & QSTO,CNEW,RETA,DTRANS,ISS,RMASIO,COLD)
C ********************************************************************
...
C
     ...
     &          CTMP,QSS,RMASIO,DELR,DELC,DH,QSTO,RETA,COLD
      LOGICAL   FWEL,FDRN,FRCH,FEVT,FRIV,FGHB,
     &          FSTR,FRES,FFHB,FIBS,FTLK,FLAK,FMNW,FDRT,FETS,FUSR(3)
      DIMENSION ICBUND(NCOL,NROW,NLAY,NCOMP),SS(7,MXSS),SSG(5,MXSS),
     &          SSMC(NCOMP,MXSS),RECH(NCOL,NROW),IRCH(NCOL,NROW),
...
C--SEAWAT: mt3d bug            CTMP=CNEW(J,I,K,ICOMP)
