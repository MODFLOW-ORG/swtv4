...
C--SEAWAT: DIMENSION ADDITIONAL ARRAYS
      DIMENSION CRCH(NCOL,NROW,NCOMP)
C     ------------------------------------------------------------------
C
...
C1A-----IF CELL IS EXTERNAL THERE IS NO RECHARGE INTO IT.
        ...
        DENSE=DENSEREF
        IF(MT3DRHOFLG.NE.0) DENSE=CALCDENS(IC,IR,1,CRCH(IC,IR,1:NCOMP))
...
C2B-----IF THE CELL IS EXTERNAL THERE IS NO RECHARGE INTO IT.
        ...
        DENSE=DENSEREF
        IF(MT3DRHOFLG.NE.0) DENSE=CALCDENS(IC,IR,IL,CRCH(IC,IR,1:NCOMP))
...
C--SEAWAT: SET DENSE
            DENSE=DENSEREF
            IF(MT3DRHOFLG.NE.0) DENSE=CALCDENS(IC,IR,IL,
     &                               CRCH(IC,IR,1:NCOMP))
            IF(RECH(IC,IR).LT.0) DENSE=PS(IC,IR,IL)
C--SEAWAT:CONSERVE MASS
C           Q=RECH(IC,IR)
