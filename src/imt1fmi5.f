C--SEAWAT: MODIFIED IMT1FMI5RP1 AND RP2 SO THAT STABILITY CRITERIA WOULD
C--SEAWAT: NOT BE ENFORCED FOR CONSTANT CONCENTRATION CELLS
C--SEAWAT: FIXED BUG IN IMT1FMIRP1 THAT WAS CAUSING PROBLEM WITH REWETTING
C--SEAWAT: 9/27/07--FMI SUBROUTINES COMPLETELY MODIFIED TO ELIMINATE THE
C--SEAWAT:   $FILE.UMT METHOD FOR PASSING FLOW INFORMATION TO MT3D.  NOW
C--SEAWAT:   ALL FLOW INFORMATION IS PASSED DIRECTLY TO MT3D ROUTINES
C--SEAWAT:   THROUGH MEMORY.
C
      SUBROUTINE IMT1FMI5AL(INFTL,IOUT,MXTRNOP,iUnitTRNOP,NPERFL,ISS,
     & IVER,ITRSS,NPER,IUNIT,CUNIT,NIUNIT,ILMTHEAD,NODES,IBOUND)
C **********************************************************************
...
C
     ...
     &          MTDRT,MTETS,MTMNW,MTUSR1,MTUSR2,MTUSR3,IERR
...
C--SEAWAT VARIABLES FROM LMT6BAS6
      INTEGER IU,ITRSS,NPER,IUNIT,NIUNIT,ILMTHEAD,INLMT,MTBCF,MTLPF,
     &        MTHUF,N,NODES,IBOUND
      CHARACTER CUNIT*4
...
C
C***************************************************************************
C--SEAWAT: PUT STATEMENTS FROM LMT6BAS6VD INTO HERE (NO MORE $FILE.UMT)
C***************************************************************************
C
      ISS=0
      IF(ITRSS.EQ.0) ISS=1
      ...
      ENDDO
C
C--SEAWAT: END OF LMT6BAS6VD STATEMENTS
C***************************************************************************

C--PRINT PACKAGE NAME AND VERSION NUMBER
C      WRITE(IOUT,1030) INFTL
C 1030 FORMAT(1X,'FMI5 -- FLOW MODEL INTERFACE PACKAGE,',
C     & ' VERSION 5, FEBRUARY 2005, INPUT READ FROM UNIT',I3)
C
C--INITIALIZE
C      ISS=1
C      NPERFL=0
...
C      IF(IFTLFMT.EQ.0) THEN
C        READ(INFTL,ERR=100,IOSTAT=IERR) VERSION,MTWEL,MTDRN,MTRCH,
C     &   MTEVT,MTRIV,MTGHB,MTCHD,ISS,NPERFL
C      ELSEIF(IFTLFMT.EQ.1) THEN
C        READ(INFTL,*,ERR=100,IOSTAT=IERR) VERSION,MTWEL,MTDRN,MTRCH,
C     &   MTEVT,MTRIV,MTGHB,MTCHD,ISS,NPERFL
C      ENDIF
C
C  100 IF(VERSION(1:4).NE.'MT3D'.OR.IERR.NE.0) THEN
C        GOTO 500
C      ELSEIF(VERSION(1:11).EQ.'MT3D4.00.00') THEN
C        REWIND(INFTL)
C        IF(IFTLFMT.EQ.0) THEN
C          READ(INFTL) VERSION,MTWEL,MTDRN,MTRCH,MTEVT,
C     &     MTRIV,MTGHB,MTCHD,ISS,NPERFL,
C     &     MTSTR,MTRES,MTFHB,MTDRT,MTETS,MTTLK,MTIBS,MTLAK,MTMNW,
C     &     MTUSR1,MTUSR2,MTUSR3
C        ELSEIF(IFTLFMT.EQ.1) THEN
C          READ(INFTL,*) VERSION,MTWEL,MTDRN,MTRCH,MTEVT,
C     &     MTRIV,MTGHB,MTCHD,ISS,NPERFL,
C     &     MTSTR,MTRES,MTFHB,MTDRT,MTETS,MTTLK,MTIBS,MTLAK,MTMNW,
...
C
C
      SUBROUTINE IMT1FMI5RP1(INUF,IOUT,KPER,KSTP,NCOL,NROW,NLAY,NCOMP,
...
C **********************************************************************
C last modified: 02-15-2005, 9/20/2007 BY LANGEVIN TO FIX REWETTING
C
      IMPLICIT  NONE
...
C--SEAWAT: ADDED FIX FOR REWETTING
                COLD(J,I,K,INDEX)=CTMP
                CNEW(J,I,K,INDEX)=CTMP
                WRITE(IOUT,122) K,I,J,INDEX,CTMP
C--SEAWAT: END OF FIX
C                CTMP=(COLD(J,I,K,INDEX)*(RETA(J,I,K,INDEX)-1.0)+CTMP)
C     &              /RETA(J,I,K,INDEX)
C                CNEW(J,I,K,INDEX)=CTMP
C                WRITE(IOUT,122) K,I,J,INDEX,CNEW(J,I,K,INDEX)
              ENDDO
            ENDIF
          ENDDO
        ENDDO
...
C
      ...
      DO K=1,NLAY
        DO I=1,NROW
          DO J=2,NCOL
C
C--SEAWAT: CHANGED FROM .NE. TO .GT.
...
C
      DO K=1,NLAY
        DO I=1,NROW
          DO J=1,NCOL
C--SEAWAT: CHANGED FROM .EQ. TO .LE.
...
C
C
C--SEAWAT: SUBROUTINE MODIFIED TO REWET CONSTANT CONCENTRATION CELLS
      SUBROUTINE IMT1FMI5RP2(INUF,IOUT,KPER,KSTP,NCOL,NROW,NLAY,NCOMP,
     & FPRT,LAYCON,ICBUND,DH,PRSITY,DELR,DELC,
     & IRCH,RECH,IEVT,EVTR,MXSS,NSS,NTSS,SS,BUFF,DTSSM,
     & SSMC,CNEW,COLD)  !SEAWAT: ADDED VARIABLES FOR REWETTING
C **********************************************************************
...
C--SEAWAT: DECLARE AND DIMENSION VARIABLES NEEDED FOR REWETTING
      REAL SSMC,CTMP,CNEW,COLD
      INTEGER ITYPE
      DIMENSION SSMC(NCOMP,MXSS),CNEW(NCOL,NROW,NLAY,NCOMP),
     +          COLD(NCOL,NROW,NLAY,NCOMP)
C--SEAWAT: CHANGED COMMON NAME FC TO FCMT3D TO AVOID CONFLICT WITH SUBROUTINE
      COMMON /FCMT3D/FWEL,FDRN,FRCH,FEVT,FRIV,FGHB,
     &          FSTR,FRES,FFHB,FIBS,FTLK,FLAK,FMNW,FDRT,FETS,FUSR
C
C--SEAWAT: THIS FIRST CHUNK HAS BEEN COMMENTED OUT AND PUT INTO FMI5RP2A, WHICH IS
C--SEAWAT: CALLED BEFORE THE LINK SUBROUTINES
...
C      ENDDO
C
C--SEAWAT: BEFORE RESETTING ICBUND, SEE IF REWET CELL IS CONSTANT CONCENTRATION
      DO K=1,NLAY
        DO I=1,NROW
          DO J=1,NCOL
            IF(ICBUND(J,I,K,1).EQ.30000) THEN
                DO NUM=1,NTSS
                    KK=SS(1,NUM)
                    II=SS(2,NUM)
                    JJ=SS(3,NUM)
                    ITYPE=SS(6,NUM)
                    IF(K.EQ.KK.AND.I.EQ.II.AND.J.EQ.JJ.AND.
     &                 ITYPE.EQ.-1) THEN
                       ICBUND(J,I,K,1)=-1
                        DO INDEX=1,NCOMP
                            CTMP=SS(4,NUM)
                            IF(NCOMP.GT.1) CTMP=SSMC(INDEX,NUM)
                            IF (CTMP.GE.0.) THEN
                                CNEW(J,I,K,INDEX)=CTMP
                                COLD(J,I,K,INDEX)=CTMP
                                WRITE(IOUT,122)K,I,J,INDEX,CTMP
        ...
        ENDDO
      ENDDO
  122 FORMAT(/1X,'DRY CONSTANT-CONCENTRATION CELL REACTIVATED AT K =',
     & I4,',   I=',I4,
     & ',   J=',I4/1X,'FOR SPECIES ',I3.3,
     & ' WITH STARTING CONCENTRATION =',G13.5)
C--SEAWAT: END OF SEAWAT CHANGES      
C
...
C--READ CONSTANT-HEAD FLOW TERM (UNIT: L**3/T).
      TEXT='CNH'
      IQ=1
C      CALL READPS(INUF,IOUT,NCOL,NROW,NLAY,KSTP,KPER,TEXT,
C     & BUFF,IQ,MXSS,NTSS,NSS,SS,ICBUND,FPRT)
C
...
C--IF RECHARGE OPTION USED IN FLOW MODEL
      IF(FRCH) THEN
        TEXT='RCH'
C        CALL READDS(INUF,IOUT,NCOL,NROW,NLAY,KSTP,KPER,TEXT,
C     &   RECH,IRCH,FPRT)
      ENDIF
C
...
C--IF MNW OPTION IS USED IN FLOW MODEL.
      IF(FMNW) THEN
        TEXT='MNW'
        IQ=27
C        CALL READGS(INUF,IOUT,NCOL,NROW,NLAY,KSTP,KPER,TEXT,
C     &   BUFF,IQ,MXSS,NTSS,NSS,SS,ICBUND,FPRT)
      ENDIF
C
...
C
      ...
      DO I=1,NROW
        DO J=1,NCOL
          ...
          ENDIF
C--SEAWAT: CHANGED FROM .EQ. TO .LE.
          IF(RECH(J,I).LE.0 .OR. ICBUND(J,I,K,1).LE.0) CYCLE
  ...
  950 IF(.NOT.FEVT .AND. .NOT.FETS) GOTO 960
      DO I=1,NROW
        DO J=1,NCOL
          ...
          ENDIF
C--SEAWAT: CHANGED FROM .EQ. TO .LE.
          IF(EVTR(J,I).EQ.0 .OR. ICBUND(J,I,K,1).LE.0) CYCLE
...
C
  960 IF(NTSS.LE.0) GOTO 990
      DO NUM=1,NTSS
        ...
        ENDIF
C--SEAWAT: CHANGED FROM .EQ. TO .LE.
        IF(SS(5,NUM).LE.0 .OR. ICBUND(J,I,K,1).LE.0) CYCLE
...
C
C
C--SEAWAT: NEW SUBROUTINES FOR PASSING FLOW INFORMATION DIRECTLY TO MT3D ROUTINES
C
      SUBROUTINE IMT1FMI5RP2A (MXSS,NSS,NTSS,SS)
C **********************************************************************
C THIS SUBROUTINE WAS CREATED SPECIFICALLY FOR SEAWAT TO CALL BEFORE THE
C LINK SUBROUTINES, WHICH NOW PUT THE FLOW RATES DIRECTLY INTO THE SS
C ARRAY.  THE FLOW RATES IN THE SS ARRAY ARE SET TO ZERO
C **********************************************************************
C last modified: 02-15-2005
C
      IMPLICIT  NONE
      INTEGER   NTSS,NSS,NUM,MXSS
      REAL SS
      DIMENSION SS(7,MXSS)
C
C--RESET TOTAL NUMBER OF POINT SINKS/SOURCES AND CLEAR FLOW RATES
...
C
C
      SUBROUTINE PASSPS(K,I,J,QSTEMP,IQ,
     &                  NCOL,NROW,NLAY,MXSS,NTSS,NSS,SS,ICBUND)
C *********************************************************************
C THIS SUBROUTINE PASSES FLOW RATES OF POINT SINK/SOURCE
C FLOW TERMS FROM THE FLOW MODEL INTO THE SS ARRAY
C *********************************************************************
C last modified: 02-15-2005
C
      IMPLICIT  NONE
      INTEGER   NCOL,NROW,NLAY,K,I,J,
     &          MXSS,NTSS,NSS,ICBUND,IQ,ID,
     &          KKK,III,JJJ,ITEMP
      REAL      SS,QSS,QSTEMP
      DIMENSION ICBUND(NCOL,NROW,NLAY),SS(7,MXSS)
C
C--IF ALREADY DEFINED AS A SOURCE OF USER-SPECIFIED CONCENTRATION,
C--STORE FLOW RATE QSTEMP
C
C--SEAWAT: K,I,J,QSTEMP,IQ ALL PASSED IN INSTEAD OF READ FROM LINK FILE
C
        DO ITEMP=1,NSS
          KKK=SS(1,ITEMP)
          III=SS(2,ITEMP)
...
C
C
      SUBROUTINE PASSGS(K,I,J,QSTEMP,IGROUP,QSW,IQ,NCOL,NROW,NLAY,
...
C
      IMPLICIT  NONE
      INTEGER   NCOL,NROW,NLAY,K,I,J,MXSS,NTSS,NSS,ICBUND,IQ,ID,
     &          KKK,III,JJJ,ITEMP,IGROUP
      REAL      SS,QSS,QSTEMP,QSW
      DIMENSION ICBUND(NCOL,NROW,NLAY),SS(7,MXSS)
C
C
C--SEAWAT: INSTEAD OF READING K,I,J,QSTEMP,IGROUP,QSW,IQ THIS INFORMATION IS PASSED
C--SEAWAT  IN FROM THE LMT SUBROUTINES
C
C--IF ALREADY DEFINED AS A SOURCE OF USER-SPECIFIED CONCENTRATION,
C--STORE FLOW RATE QSTEMP                               
        DO ITEMP=1,NSS
