C  A bug in the subroutine SGWF1HUF2HDOT (which interpolates heads
C  calculated in cells to heads in hydrogeologic units) is fixed
C  by changing the outer loop to sort HGUs in all rows and columns.
C  CEH 5/20/2009
C 
...
C=======================================================================
     ...
     &  NHUF,NPHUF,LCHUFTHK,LCHKCC,ISUMI,IOHUFHDS,IOHUFFLWS,
     &  LAYHDT,LCHUFTMP)
C
C     ******************************************************************
...
C1------IDENTIFY PACKAGE
    ...
    1 FORMAT(1X,/1X,'HUF2 -- HYDROGEOLOGIC-UNIT FLOW PACKAGE,
     & VERSION 2.0, 07/02/2003',/,' INPUT READ FROM UNIT',I3,/)
C
...
C8------RETURN.
      ...
      END
C=======================================================================
      SUBROUTINE GWF1HUF2RPGD(
     & IN,NCOL,NROW,NLAY,IOUT,WETDRY,WETFCT,IWETIT,IHDWET,
     & IHGUFLG,ITERP,NHUF,NPHUF,HUFTHK,ITRSS)
C
C     ******************************************************************
...
C2H-----(LAYWT NOT 0).
  ...
  300 CONTINUE
      ...
      WRITE(IOUT,47)
   47 FORMAT(
     & //1X,'HUF2 -- HYDROGEOLOGIC-UNIT FLOW PACKAGE',
     & /1X,75('-'))
C
C-------READ HYDROGEOLOGIC-UNIT GEOMETRY
      Call SGWF1HUF2GEOMRP(IN,NCOL,NROW,IOUT,NHUF,HUFTHK)
C
...
C  Find the unit name in the list
  ...
  201   CONTINUE
   ...
   41     FORMAT('UNIT ',A10,'NOT FOUND (STOP EXECUTION(GWF1HUF2RPGD))')
          CALL USTOP(' ')
        ENDIF
  ...
  101 CONTINUE
   ...
   48 FORMAT(
     ...
     & /1X,75('-'))
      DO 210 K=1,NHUF
        LAYPRN=VKANAM(1)
        IF(HGUVANI(K).NE.0.0) LAYPRN=VKANAM(2)
...
C-------READ NAMED PARAMETERS
      ...
      IF(NPHUF.GT.0) THEN
        DO 20 K=1,NPHUF
          CALL UHUF2PARRP(IN,IOUT,NP,PTYP,ITERP,NHUF)
          IF(PTYP.EQ.'HK') THEN
            CONTINUE
...
C  Make some basic parameter checks
  ...
  500   FORMAT(//3X,
     ...
     &    'defined in the HUF Package (STOP GWF1HUF2RPGD)')
        CALL USTOP(' ')
      ENDIF
C
...
C
  ...
  530   FORMAT(//3X,
     ...
     &    'convertible layer.  SYTP will be disregarded (GWF1HUF2RPGD)')
      ENDIF
...
c======================================================================
      SUBROUTINE SGWF1HUF2GEOMRP(IN,NCOL,NROW,IOUT,NHUF,HUFTHK)
C
C     ******************************************************************
...
C Check for cells that GO DRY/REWET
      DO 5 K=1,NLAY
        CALL SGWF1HUF2WETCHK(HNEW,IBOUND,CC,BOTM,
...
C
     ...
     &                      VKA,HNEW,IBOUND,HUFHK,HUFVK,NHUF,NU,
     &                      HUFKDEP(NU),IOUT,GS)
...
C-------CALCULATE CR AND CC
        CALL SGWF1HUF2HCND(HNEW,IBOUND,CR,CC,HK,DELR,
...
C     ------------------------------------------------------------------
     ...
     &  HNEW(NCOL,NROW,NLAY),GS(NCOL,NROW)
      ...
      MULTKDEP = 1.0
      IF(KT.EQ.KB) THEN
        IF(LAMBDA.NE.0.) CALL SGWF1HUF2KDEP(LAMBDA,TOPU,BOTU,
     &                                      GS(J,I),MULTKDEP)
...
C---Check for small or negative thickness
          ...
          IF(LAMBDA.NE.0.) CALL SGWF1HUF2KDEP(LAMBDA,TOP1,BOT1,
     &                                        GS(J,I),MULTKDEP)
...
c======================================================================
      SUBROUTINE SGWF1HUF2VKL(
     & VK,KL,NCOL,NROW,NLAY,BOTM,NBOTM,HNEW,IBOUND,IOUT,GS,HUFTHK,IZON,
     & NZONAR,RMLT,NMLTAR,NHUF)
C
C     ******************************************************************
...
C     ------------------------------------------------------------------
      ...
      REAL MULTKDEP
      DIMENSION HNEW(NCOL,NROW,NLAY),BOTM(NCOL,NROW,0:NBOTM),
     &  VK(NCOL,NROW),HUFHK(999),HUFVK(999),HUFKDEP(999),
...
C           GET THICKNESS OF UNIT IN THIS LAYER
            CALL UHUF2THK(BOTM(J,I,LBOTM(KL)-1),BOTM(J,I,LBOTM(KL)),
     ...
     &                          HUFTHK,IZON,NZONAR,RMLT,NMLTAR,NU,IOUT)
              IF(HUFKDEP(NU).NE.0.) CALL SGWF1HUF2KDEP(HUFKDEP(NU),TOPU,
     &                                                 BOTU,GS(J,I),
     &                                                 MULTKDEP)
...
c======================================================================
      SUBROUTINE SGWF1HUF2VSRCH(NCOL,NROW,NLAY,BOTM,NBOTM,I,J,TOPU,BOTU,
     &  HNEW,IBOUND,KT,KB,IFLG)
C
C     ******************************************************************
...
C4------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2HSRCH(NCOL,NROW,NLAY,BOTM,NBOTM,I,J,TOPU,BOTU,
     &  HNEW,IBOUND,KT,KB,IFLG)
C
C     ******************************************************************
C     Search for top and bottom layer the unit applies to.
...
C4------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2SC1(NCOL,NROW,NLAY,BOTM,NBOTM,I,J,TOPU,BOTU,
...
C
     ...
     &    HK(NCOL,NROW,NLAY),VKA(NCOL,NROW,NLAY),
...
c======================================================================
      SUBROUTINE SGWF1HUF2HCND(HNEW,IBOUND,CR,CC,HK,DELR,DELC,
...
C-------CONVERT HK TO HYDRAULIC CONDUCTVITY AND SC1 TO SPECIFIC STORAGE
   ...
   71   CONTINUE
      KK=K
        CALL SGWF1HUF2HCOND(HNEW,IBOUND,CR,CC,HK,DELR,DELC,BOTM,
...
C7------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VCND(IBOUND,CV,VKA,DELR,DELC,
     &  NCOL,NROW,NLAY)
C
C     ******************************************************************
...
C7------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2SYTP(SC1,IBOUND,NCOL,NROW,NLAY,IZON,NZONAR,
     &  RMLT,NMLTAR)
C
C     ******************************************************************
...
C7------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2SC(IBOUND,SC1,DELR,DELC,NCOL,NROW,NLAY)
C
C     ******************************************************************
C     CALCULATE SC1
C     ******************************************************************
...
C7------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2HCOND(HNEW,IBOUND,CR,CC,HK,DELR,DELC,BOTM,
...
C3C-----HNEW=HDRY, SATURATED THICKNESS=0.0, AND IBOUND=0.
         IF(THCK.LE.ZERO) THEN
            CALL SGWF1HUF2WDMSG(1,NCNVRT,ICNVRT,JCNVRT,ACNVRT,IHDCNV,
...
C4------PRINT ANY REMAINING CELL CONVERSIONS NOT YET PRINTED.
      CALL SGWF1HUF2WDMSG(0,NCNVRT,ICNVRT,JCNVRT,ACNVRT,IHDCNV,
     &             IOUT,KITER,J,I,K,KSTP,KPER)
C
...
C6------CONDUCTIVITY, SATURATED THICKNESS, AND GRID DIMENSIONS.
         CALL SGWF1HUF2HHARM(CR,CC,HK,IBOUND,DELR,DELC,K,NCOL,NROW,
     &         NLAY,HKCC)
C
C7------RETURN.
      RETURN
...
c======================================================================
      SUBROUTINE SGWF1HUF2WETCHK(HNEW,IBOUND,CC,BOTM,
     & NBOTM,K,KITER,KSTP,KPER,NCOL,NROW,NLAY,IOUT,WETDRY,
     & WETFCT,IWETIT,IHDWET,HDRY)
C     ******************************************************************
C     CHECK FOR CELLS THAT GO DRY/REWET
...
C2------IF LAYER IS WETTABLE CONVERT DRY CELLS TO WET WHEN APPROPRIATE.
      ...
      IF(ITFLG.EQ.0) CALL SGWF1HUF2WET(HNEW,IBOUND,BOTM,NBOTM,K,KITER,
...
c======================================================================
      SUBROUTINE GWF1HUF2AD(IBOUND,HOLD,BOTM,WETDRY,ISS,NCOL,NROW,NLAY,
     &            NBOTM)
C
C%%%%%
...
C3-----RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2WET(HNEW,IBOUND,BOTM,NBOTM,K,KITER,KSTP,KPER,
...
C7------IBOUND.
   50    CALL SGWF1HUF2WDMSG(2,NCNVRT,ICNVRT,JCNVRT,ACNVRT,IHDCNV,
     &             IOUT,KITER,J,I,K,KSTP,KPER)
C
...
c======================================================================
      SUBROUTINE SGWF1HUF2WDMSG(ICODE,NCNVRT,ICNVRT,JCNVRT,ACNVRT,
...
c======================================================================
      SUBROUTINE SGWF1HUF2HHARM(CR,CC,HK,IBOUND,DELR,DELC,K,NCOL,NROW,
     &         NLAY,HKCC)
C
C     ******************************************************************
...
c======================================================================
      SUBROUTINE GWF1HUF2FM(HCOF,RHS,HOLD,SC1,HNEW,IBOUND,CR,CC,CV,HK,
...
C---------------Compute SC2 Component
                CALL SGWF1HUF2SC2(0,J,I,K,TOP,BOT,HN,HO,TLED,CHCOF,CRHS,
...
C9------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2SC2(IFLG,J,I,K,TOP,BOT,HN,HO,TLED,CHCOF,CRHS,
     &                    HUFTHK,NCOL,NROW,NHUF,IZON,NZONAR,RMLT,
     &                    NMLTAR,AREA,IOUT)
C
C     ******************************************************************
...
C4------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2S(VBNM,VBVL,MSUM,HNEW,IBOUND,HOLD,SC1,
...
C11----RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2B(HNEW,IBOUND,CR,CC,CV,BOTM,NCOL,NROW,NLAY,
...
C5C-----RECORD CONTENTS OF BUFFER AND RETURN.
     ...
     &               NLAY,IOUT,DELT,PERTIM,TOTIM,IBOUND)
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2F(VBNM,VBVL,MSUM,HNEW,IBOUND,CR,CC,CV,BOTM,
     &         DELT,NCOL,NROW,NLAY,KSTP,KPER,IHUFCB,BUFF,IOUT,ICBCFL,
     &         PERTIM,TOTIM,NBOTM,ICHFLG,ILVDA,VDHT)
C     ******************************************************************
...
C18-----RETURN.
      ...
      END
c======================================================================
      SUBROUTINE GWF1HUF2OT(IOHUFHDS,IOHUFFLWS,HNEW,IHEDFM,IBOUND,NHUF,
...
C-------CALCULATE HEADS WITHIN UNITS
      IF(IOHUFHDS.GT.0) THEN
        CALL SGWF1HUF2HDOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
...
C-------CALCULATE FLOWS WITHIN UNITS
      ...
      CALL SGWF1HUF2CHFLOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
     &                   HUFTHK,BOTM,NBOTM,CV,DELR,DELC,RMLT,NMLTAR,
     &                   IZON,NZONAR,IOUT,GS,IOHUFFLWS,ICBCFL,KSTP,KPER,
     &                   ICHFLG)
      CALL SGWF1HUF2FLOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
     &                   HUFTHK,BOTM,NBOTM,CV,DELR,DELC,RMLT,NMLTAR,
     &                   IZON,NZONAR,IOUT,GS,IOHUFFLWS,ICBCFL,KSTP,KPER,
...
C6------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2FLOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
     &                   HUFTHK,BOTM,NBOTM,CV,DELR,DELC,RMLT,NMLTAR,
...
C6------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2HDOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
     &                   HUFTHK,BOTM,NBOTM,CV,DELR,DELC,RMLT,NMLTAR,
     &                   IZON,NZONAR,HNOFLO,IOUT,GS)
C
C     ******************************************************************
...
C6------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2CHFLOT(HNWHGU,HNEW,IBOUND,NHUF,NCOL,NROW,NLAY,
     &                   HUFTHK,BOTM,NBOTM,CV,DELR,DELC,RMLT,NMLTAR,
     &                   IZON,NZONAR,IOUT,GS,IHUFCB,IBD,KSTP,KPER,
     &                   ICHFLG)
C     ******************************************************************
...
C
     ...
     &    RMLT(NCOL,NROW,NMLTAR),IZON(NCOL,NROW,NZONAR),
     &    HUFTHK(NCOL,NROW,NHUF,2),HUFHK(999),HUFHANI(999),
     &    HUFKDEP(999),GS(NCOL,NROW)
C
C      INCLUDE 'param.inc'
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
      COMMON /HUFCOM/HGUHANI(999),HGUVANI(999),LTHUF(999),LAYWT(999)
...
C---Get thickness of unit within cell
        ...
        CALL UHUF2THK(TOP0,BOT0,TOPU,THCKU,THCK,ATPU,ABTU)
...
C---Calculate transmissivity for cell that pinches out
          ...
          DO 200 NU =1,NHUF
            ...
            CALL UHUF2THK(TOPG,BOTG,TOPUU,THCKUU,THK,ATPU,ABTU)
     ...
     &              HUFHANI(NU)=HGUHANI(NU)
              MULTKDEP = 1.0
              LAMBDA = HUFKDEP(NU)
              IF(LAMBDA.NE.0.) CALL SGWF1HUF2KDEP(LAMBDA,TOPUU,BOTUU,
     &                                            GS(J,I),MULTKDEP)
...
C---Otherwise, calculate transmissivities of cell
     ...
     &          HUFHANI(GPT)=HGUHANI(GPT)
          MULTKDEP = 1.0
          LAMBDA = HUFKDEP(GPT)
          CALL UHUF2THK(TOP0,BOT0,TOPU,THCKU,THK,ATPU,ABTU)
          IF(LAMBDA.NE.0.) CALL SGWF1HUF2KDEP(LAMBDA,ATPU,ABTU,
     &                                        GS(J,I),MULTKDEP)
...
C
c======================================================================
      SUBROUTINE SGWF1HUF2IND(NCOL,NROW,NHUF,HUFTHK,JJ,II,INDX)
C
C     ******************************************************************
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      INTEGER NHUF,indx(999),M,NSTACK
...
5       indx(l+1)=indx(j)
      ...
      END
c======================================================================
      SUBROUTINE GWF1HUF2KDEP1ALG(ISUM,IN,IOUT,NCOL,NROW,
     &  LCGS,NPKDEP,IFKDEP)
C
C     ******************************************************************
C     ALLOCATE ARRAY STORAGE FOR KDEP PARAMETER
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C1------IDENTIFY PACKAGE
      WRITE(IOUT,1) IN
    1 FORMAT(1X,/1X,'KDEP1 -- HYDRAULIC CONDUCTIVITY DEPTH-DECAY
     & VERSION 2.0, 07/02/2003',/,' INPUT READ FROM UNIT',I3,/)
C
C2------READ FIRST RECORD AND WRITE
      CALL URDCOM(IN,IOUT,LINE)
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NPKDEP,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IFKDEP,R,IOUT,IN)
C
C  Allocate space in parameter-information arrays
...
C5------COMPUTE THE NUMBER OF CELLS IN THE ENTIRE GRID AND IN ONE LAYER.
      NRC=NROW*NCOL
C
C6------ALLOCATE SPACE FOR ARRAY.
      ISOLD=ISUM
      LCGS=ISUM
      ISUM=ISUM+NRC
C7------PRINT THE AMOUNT OF SPACE USED BY THE KDEP CAPABILITY.
      ISP=ISUM-ISOLD
      WRITE(IOUT,101) ISP
  101 FORMAT(1X,/1X,I10,' ELEMENTS IN X ARRAY ARE USED BY KDEP')
C
C8------RETURN.
      RETURN
      END
C=======================================================================
      SUBROUTINE GWF1HUF2KDEP1RPGD(
...
C4------RETURN
      ...
      END
c======================================================================
      SUBROUTINE GWF1HUF2LVDA1ALG(ISUM,ISUMI,IN,IOUT,NCOL,NROW,
     &  NLAY,LCVDHD,LCDVDH,LCVDHT,NPLVDA,LCA9)
C
C     ******************************************************************
C     ALLOCATE ARRAY STORAGE FOR LVDA CAPABILITY
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
C
C-------ALLOCATE SPACE FOR VARIABLE DIRECTION HORIZONTAL ANISOTROPY ARRAYS
      IF(IN.EQ.0) THEN
        LCVDHD=1
        LCVDHT=1
        LCDVDH=1
...
C1------IDENTIFY PACKAGE
      WRITE(IOUT,1) IN
    1 FORMAT(1X,/1X,'LVDA1 -- MODEL-LAYER VARIABLE-DIRECTION
...
C2------READ FIRST RECORD AND WRITE
      CALL URDCOM(IN,IOUT,LINE)
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NPLVDA,R,IOUT,IN)
C
C  Allocate space in parameter-information arrays
...
C6------ALLOCATE SPACE FOR ARRAYS.
      ...
      LCA9=ISUM
      ISUM=ISUM+ISIZ*5
C7------PRINT THE AMOUNT OF SPACE USED BY THE LVDA CAPABILITY.
      ISP=ISUM-ISOLD
      WRITE(IOUT,101) ISP
  101 FORMAT(1X,/1X,I10,' ELEMENTS IN X ARRAY ARE USED BY LVDA')
      ISP=ISUMI-ISOLDI
      WRITE(IOUT,102) ISP
  102 FORMAT(1X,I10,' ELEMENTS IN IX ARRAY ARE USED BY LVDA')
C
C8------RETURN.
      RETURN
      END
C=======================================================================
      SUBROUTINE GWF1HUF2LVDA1RPGD(
...
C     LAYERS
   ...
   30   CONTINUE
     ...
     & ' convertible layers!'
          WRITE(IOUT,*) ' STOP EXECUTION - GWF1HUF2LVDA1RPGD'
          CALL USTOP(' ')
        ENDIF
      ENDIF
C
...
C-------and subtract elements from RHS
              CALL SGWF1HUF2VDA9F9(I,J,K,A9,HNEW,IBOUND,NLAY,NROW,NCOL,
     &                             TF9)
              H1 = HNEW(J,I,K)
              IF(I.GT.1) THEN
                CCT = CC(J,I-1,K)
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDHHV(IBOUND,HK,HKCC,VDHD,NCOL,NROW,NLAY,
     &  HUFTHK,NHUF,BOTM,NBOTM,IZON,NZONAR,RMLT,NMLTAR,HNEW,GS)
C
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     &    IBOUND(NCOL,NROW,NLAY),HUFHK(999),HUFHANI(999),
...
C---Populate HGU arrays for given i,j depending on parameter type
     ...
     &                    IZON,NZONAR,RMLT,NMLTAR,0,IOUT)
        CALL UHUF2POP(HUFKDEP,'KDEP',NCOL,NROW,NHUF,I,J,HUFTHK,
     &                    IZON,NZONAR,RMLT,NMLTAR,0,IOUT)
        CALL UHUF2POPL(VDHD,NCOL,NROW,NLAY,I,J,
     &                        IZON,NZONAR,RMLT,NMLTAR)
C
C---Populate MODEL arrays
        DO 130 NU=1,NHUF
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDF9(I,J,K,VDHT,HNEW,IBOUND,NLAY,NROW,NCOL,
     &                         FL,FR,FT,FB)
C     ******************************************************************
...
C-------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDA9F9(I,J,K,A9,HNEW,IBOUND,NLAY,NROW,NCOL,F)
C     ******************************************************************
...
C-------RETURN.
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDA9(VDHT,A9,IBOUND,NLAY,NROW,NCOL)
C     ******************************************************************
...
C-------Local cell 4, I+1,J
     ...
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAS2(AS21234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAS3(AS31234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAS4(AS41234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAW1(AW11234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAW2(AW21234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAW3(AW31234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
      CALL SGWF1HUF2VDCOFAW4(AW41234,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                       TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D1234)
...
C-------Local cell 6, I,J-1
      ...
      ELSE
        ...
        TBB6 = TBB1/TMPCOF
      ENDIF
      CALL SGWF1HUF2VDCOFD(D6145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                     TAA4,TAB4,TBB4,TAA5,TAB5,TBB5)
      CALL SGWF1HUF2VDCOFAS1(AS16145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAS2(AS26145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAS3(AS36145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAS4(AS46145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAE1(AE16145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAE2(AE26145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAE3(AE36145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
      CALL SGWF1HUF2VDCOFAE4(AE46145,TAA6,TAB6,TBB6,TAA1,TAB1,TBB1,
     &                       TAA4,TAB4,TBB4,TAA5,TAB5,TBB5,D6145)
C
C
C-------RETURN
...
C-------Local cell 8, I-1,J
      ...
      ELSE
        ...
        TBB8 = TBB1/TMPCOF
      ENDIF
      CALL SGWF1HUF2VDCOFD(D7816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                     TAA1,TAB1,TBB1,TAA6,TAB6,TBB6)
      CALL SGWF1HUF2VDCOFAN1(AN17816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAN2(AN27816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAN3(AN37816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAN4(AN47816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAE1(AE17816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAE2(AE27816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAE3(AE37816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
      CALL SGWF1HUF2VDCOFAE4(AE47816,TAA7,TAB7,TBB7,TAA8,TAB8,TBB8,
     &                       TAA1,TAB1,TBB1,TAA6,TAB6,TBB6,D7816)
C
C
C-------RETURN
...
C-------Local cell 9, I-1,J+1
      ...
      ELSE
        ...
        TBB9 = TBB1/TMPCOF
      ENDIF
      CALL SGWF1HUF2VDCOFD(D8921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                     TAA2,TAB2,TBB2,TAA1,TAB1,TBB1)
      CALL SGWF1HUF2VDCOFAN1(AN18921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAN2(AN28921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAN3(AN38921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAN4(AN48921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAW1(AW18921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAW2(AW28921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAW3(AW38921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
      CALL SGWF1HUF2VDCOFAW4(AW48921,TAA8,TAB8,TBB8,TAA9,TAB9,TBB9,
     &                       TAA2,TAB2,TBB2,TAA1,TAB1,TBB1,D8921)
C
C
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFD(D,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAS1(AS1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAS2(AS2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAS3(AS3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAS4(AS4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAW1(AW1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAW2(AW2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAW3(AW3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAW4(AW4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAE1(AE1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAE2(AE2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAE3(AE3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAE4(AE4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAN1(AN1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAN2(AN2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAN3(AN3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
...
C-------RETURN
      ...
      END
c======================================================================
      SUBROUTINE SGWF1HUF2VDCOFAN4(AN4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &                          TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D)
C
C     ******************************************************************
