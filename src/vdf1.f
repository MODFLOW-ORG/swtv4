...
C     USE GLOBAL,         ONLY: IOUT,NCOL,NROW,NLAY
     ...
     2                       FIRSTDT,DNSCRIT,DRHODPRHD,PRHDREF,HDRY,
...
C------STORE NCOL, NROW, AND NLAY IN THE VDFMODULE
      ALLOCATE(VDFNCOL,VDFNROW,VDFNLAY)
      VDFNCOL=NCOL
      VDFNROW=NROW
      VDFNLAY=NLAY
C
...
C
     ...
     &                  MIXELM,HDRYMFLW,HNOFLOMFLW,INBTN,MXPRS,TIMPRS,
     &                  NPRS)
C     ******************************************************************
...
C
     ...
     &          HNEW(NCOL,NROW,NLAY),TIMPRS(MXPRS)
C
C     ------------------------------------------------------------------
C
C       INITIALIZE ELEVATION ARRAY
...
C--SEAWAT: CHECK FOR A TIMPRS VALUE OF ZERO AND STOP IF DETECTED
      IF(INBTN.GT.0.AND.MT3DRHOFLG.NE.0) THEN
          DO I=1,NPRS
            IF(TIMPRS(I).EQ.0.0) THEN
                WRITE(IOUT,100)
                WRITE(*,100)
                CALL USTOP('')
            ENDIF
...
C
     ...
     &  DELR,DELC,DH,DELT,IBOUND,CINACT)
C **********************************************************************
...
C************************************************************************
C   FUNCTION SSMDENSE:
C   RETURNS THE FLUID DENSITY FOR THE SPECIFIED MT3DMS BOUNDARY 
C   MODIFIED 8/18/2006 BY CDL
C   MODIFIED 9/18/2012 BY CDL TO HANDLE MT3DMS RECIRC OPTION
C************************************************************************
      USE VDFMODULE,   ONLY: DENSEREF,PS,NCOL,NROW
C
...
C
      ...
      IF (NSS.LE.0) RETURN
C
C-----SEARCH THROUGH SS ARRAY TO FIND RECORD
      ISS=0
      NSLOOP: DO NS=1,NSS
        ITYPE=SS(6,NS)
        KK=SS(1,NS)
      ...
      ENDDO NSLOOP
C
C-----RETURN WITH DENSITY EQUAL TO DENSEREF IF NO CORRESPONDING RECORD
C-----IN SS ARRAY
      IF(ISS.EQ.0) RETURN
C
      CTMP=SSMC(1,ISS)
      QSS=SS(5,ISS)
      IQ=SS(6,ISS)
C-----CHECK IF ANY SOURCE CONCENTRATIONS ARE NEGATIVE.  THIS IS NEEDED
C-----FOR WELL RECIRCULATION OPTION BELOW.
      NEGCONC=.FALSE.
      DO II=1,NCOMP
          IF(SSMC(II,ISS).LT.0) THEN
              NEGCONC=.TRUE.
              MHOST=-INT(SSMC(II,ISS))
          ENDIF
      ENDDO
      IF(IQ.EQ.2.AND.NEGCONC) THEN
C-----CALCULATE DENSITY FOR MT3D RECIRC OPTION. USE CELL DENSITY IF ANY 
C-----NEGATIVE CONC IN SSMC
          KHOST=(MHOST-1)/(NCOL*NROW) + 1
          IHOST=MOD((MHOST-1),NCOL*NROW)/NCOL + 1
          JHOST=MOD((MHOST-1),NCOL) + 1
          SSMDENSE=PS(JHOST,IHOST,KHOST)
      ELSE
C-----CALCULATE THE DENSITY OF A STANDARD SINK OR SOURCE
          SSMDENSE=CALCDENS(J,I,K,SSMC(1:NCOMP,ISS))
      ENDIF
C
      RETURN      
      END
...
C--SEAWAT:**********************************************************
      USE VDFMODULE,   ONLY: DNSCRIT,PS,COLDFLW,NSWTCPL
C
      DIMENSION ICBUND(NCOL,NROW,NLAY),CNEW(NCOL,NROW,NLAY,NCOMP),
...
C
C--NEED TO RESET DELT BECAUSE FLOW FIELD WAS JUST UPDATED
      IF(IUPDTFLWFLD.EQ.1) DELT=0.
      IUPDTFLWFLD=0
C--CASE 1.
      IF(N.EQ.1) THEN
        DELT=DTRANS
