...
C1------FOR EACH LAYER: IF CONVERTIBLE, CALCULATE CONDUCTANCES.
      DO 100 K=1,NLAY
      KK=K
C--SEAWAT: THE FOLLOWING ROUTINE ALWAYS NEEDS TO BE CALLED IF VARIABLE
C--SEAWAT: VISCOSITY IS ACTIVE AND IT IS THE FIRST ITERATION
C--SEAWAT:      IF(LAYTYP(K).NE.0)
          IF (INVSC.GT.0.AND.KITER.EQ.1 .OR. LAYTYP(K).NE.0)
     1    CALL SVDF1LPF1HCOND(HNEW,IBOUND,CR,CC,HK,HANI,DELR,DELC,BOTM,
     2      NBOTM,KK,KITER,KSTP,KPER,NCOL,NROW,NLAY,IOUT,WETDRY,
     3      WETFCT,IWETIT,IHDWET,HDRY,HSALT,INVSC)
  100 CONTINUE
      DO 101 K=1,NLAY
      KK=K
      IF(K.NE.NLAY) THEN
C--SEAWAT: THE FOLLOWING ROUTINE ALWAYS NEEDS TO BE CALLED IF VARIABLE
C--SEAWAT: VISCOSITY IS ACTIVE AND IT IS THE FIRST ITERATION
C--SEAWAT:         IF(LAYTYP(K).NE.0 .OR. LAYTYP(K+1).NE.0)
C--SEAWAT: PASSING IN HSALT INSTEAD OF HNEW (04/13/2007)
         IF(INVSC.GT.0.AND.KITER.EQ.1)      
     2      CALL SVDF1LPF1VCOND(CV,HK,VKA,VKCB,IBOUND,BOTM,NBOTM,KK,
     3         NCOL,NROW,NLAY,HSALT,DELR,DELC,IOUT)
         IF(INVSC.EQ.0 .AND. (LAYTYP(K).NE.0 .OR. LAYTYP(K+1).NE.0))
     2      CALL SGWF1LPF1VCOND(CV,HK,VKA,VKCB,IBOUND,BOTM,NBOTM,KK,
     3         NCOL,NROW,NLAY,HSALT,DELR,DELC,IOUT)
...
C9------RETURN
     ...
     2 WETFCT,IWETIT,IHDWET,HDRY,HSALT,INVSC)
...
C--SEAWAT: CALL VDF VERSION IF VARIABLE VISCOSITY
         IF(INVSC.GT.0) THEN
            CALL SVDF1LPF1HHARM(CR,CC,HK,HANI,IBOUND,DELR,DELC,K,NCOL,
     1         NROW,NLAY)
         ELSE      
            CALL SGWF1LPF1HHARM(CR,CC,HK,HANI,IBOUND,DELR,DELC,K,NCOL,
     1         NROW,NLAY)
         ENDIF
      ELSE IF(LAYAVG(K).EQ.1) THEN
C--SEAWAT: CALL VDF VERSION IF VARIABLE VISCOSITY
         IF(INVSC.GT.0) THEN
            CALL SVDF1LPF1HLOG(IBOUND,CR,CC,HK,HANI,DELR,DELC,K,NCOL,
     1        NROW,NLAY)
         ELSE
...
C--SEAWAT: CALL VDF VERSION IF VARIABLE VISCOSITY
         IF(INVSC.GT.0) THEN
            CALL SVDF1LPF1HUNCNF(IBOUND,CR,CC,HK,HANI,DELR,DELC,K,NCOL,
     1        NROW,NLAY)
         ELSE
...
C--SEAWAT: ADD HTMP,HSING
      DOUBLE PRECISION HNEW,STOIN,STOUT,SSTRG,HTMP,HSING
C
      DIMENSION HNEW(NCOL,NROW,NLAY), IBOUND(NCOL,NROW,NLAY),
...
C11----RETURN.
     ...
     2         PERTIM,TOTIM,NBOTM,ICHFLG,DELR,DELC)
...
C18-----RETURN.
     ...
     2      IDIR,IBDRET,ICHFLG,IC1,IC2,IR1,IR2,IL1,IL2,NBOTM,DELR,DELC)
C
...
C--SEAWAT DIMENSION VARIABLE DENSITY FLOW ARRAYS
      DIMENSION DELR(NCOL),DELC(NROW)
C     ------------------------------------------------------------------
C
...
C5C-----RECORD CONTENTS OF BUFFER AND RETURN.
     ...
     1     NLAY,IOUT,DELT,PERTIM,TOTIM,IBOUND)
      ...
      END
C
      SUBROUTINE SVDF1LPF1VCOND(CV,HK,VKA,VKCB,IBOUND,BOTM,NBOTM,K,
...
C      SPECIFICATIONS:
C--SEAWAT:
      USE VDFVSCMODULE,   ONLY: VISCREF,MU
C     ------------------------------------------------------------------
...
C--SEAWAT: MULTIPLY BOVK1 BY MU(J,I,K)
               BOVK1=BOVK1*MU(J,I,K)
C
C5------CALCULATE INVERSE LEAKANCE FOR CELL BELOW.
               ...
               END IF
               BOVK2=B/HYC2
C--SEAWAT: MULTIPLY BOVK2 BY MU(J,I,K+1)
               BOVK2=BOVK2*MU(J,I,K+1)               
C
               IF(LAYCBD(K).NE.0) THEN
C6------CALCULATE VERTICAL HYDRAULIC CONDUCTIVITY FOR CONFINING BED.
C--SEAWAT: STOP IF USING QUASI-3D REPRESENTATION (NOT SUPPORTED)
                  print *, 'Quasi-3D mechanism is not supported by'
     &                   , 'variable viscosity case. Stopping.'
                  STOP                  
...
C--SEAWAT: MULTIPLY BY REFERENCE VISCOSITY
                  CV(J,I,K)=CV(J,I,K)*VISCREF
               END IF
            END IF
         END IF
      END IF
...
C8------RETURN.
      ...
      END
C
      SUBROUTINE SVDF1LPF1HHARM(CR,CC,HK,HANI,IBOUND,DELR,DELC,K,NCOL,
     1         NROW,NLAY)
C
...
C--SEAWAT: MULTIPLY BY VISCREF/MU TO INCLUDE VARIABLE VISCOSITY
         T1=T1*VISCREF/MU(J,I,K)         
...
C--SEAWAT: MULTIPLY BY VISCREF/MU TO INCLUDE VARIABLE VISCOSITY
               ...
               ELSE
                  ...
                  T2=T2*CHANI(K)
               END IF
C--SEAWAT: VARIABLE VISCOSITY INCLUDED THROUGH T1 AND T2
...
C4------RETURN
      RETURN
      END
      SUBROUTINE SVDF1LPF1HLOG(IBOUND,CR,CC,HK,HANI,DELR,DELC,K,NCOL,
     1       NROW,NLAY)
C
...
C4------RETURN
      RETURN
      END
      SUBROUTINE SVDF1LPF1HUNCNF(IBOUND,CR,CC,HK,HANI,DELR,DELC,K,NCOL,
     1        NROW,NLAY)
C
